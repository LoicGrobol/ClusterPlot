...
class HandlerLineImage(HandlerBase):
    ...
    def create_artists(
        ...
        line = matplotlib.lines.Line2D(
            [xdescent + self.offset, xdescent + (width - self.space) / 3.0 + self.offset],
...
class ClusterPlot:
    def __init__(
        self,
        n_components: int = 2,
        anchors_method: str = "birch",
        n_intra_anchors: int = None,
        birch_threshold: float = 0.3,
        birch_branching_factor: int = None,
        dim_reduction_algo: str = "umap",
        supervised: bool = False,
        umap_n_neighbors: int = 15,
        umap_min_dist: int = 1,
        tsne_perplexity: float = 30.0,
        reduce_all_points: bool = False,
        ...
        top_greedy: int = 1,
        magnitude_step: bool = False,
        n_iter: int = 10,
        batch_size: int = 1,
        stop_criteria: float = 0.00001,
        loss: str = "Linf",
        only_inter_relations: bool = False,
        learning_rate: float = None,
        mask_sparse_subcluster: int = None,
        random_points_method: str = "voronoi",
        class_to_label: dict = None,
        ...
        alpha: float = None,
        remove_outliers_k: float = None,
        douglas_peucker_tolerance: float = 0.6,
        smooth_iter: int = 3,
        skip_polygons_with_area: float = 0.01,
        mask_relation_in_same_label: bool = True,
        save_fig_every: int = 1,
        ...
        main_plot_fig_size: tuple = (26, 13),
        show_anchors_annotation: bool = False,
        vmax_overlap: float = 0.5,
        vmax_proximity: float = 0.5,
        annotate_images: bool = False,
        k_annot_clf: int = 40,
        orig_images: list = None,
        ...
        if dim_reduction_algo not in supported_dim_reduction_algos:
            raise Exception(
                f"Unsupported dim_reduction_algo {dim_reduction_algo}, "
        ...
        if anchors_method not in supported_anchors_methods:
            raise Exception(
                f"Unsupported anchors method {anchors_method}, "
        ...
        if random_points_method not in supported_random_points_method:
            raise Exception(
                f"Unsupported random_points_method {random_points_method}, "
        ...
        if anchors_method in ["birch"]:
            if n_intra_anchors is not None:
                self.logger.warning(
                    f"Anchors method {anchors_method} does not require number of anchors "
        ...
        if uniform_points_per not in supported_uniform_points_per:
            raise Exception(
                f"Unsupported uniform_points_per {uniform_points_per} , "
        ...
        self.k = (
            k  # note that in t-sne paper when they presented this method they used k=20 for mnist
        )
    ...
    def fit_transform(self, X, y):
        ...
        self.anchors_indices = [
            i
            for i in range(
                len(self.y_with_centroids) - len(self.intra_class_anchors_labels),
    ...
    def _get_intra_class_anchors(self, X, y):
        ...
        if self.anchors_method == "agglomerative":
            ...
            cm = Birch(
                threshold=self.birch_threshold, branching_factor=branching_factor, n_clusters=None
            )
        ...
        for label in sorted(df[self.label_col].unique()):  # Assumed to be integers 0,1,2,...
            ...
            if self.anchors_method == "birch":
                ...
                subcluster_centers = (
                    df[df[self.label_col] == label]
                    .groupby([self.label_col, self.cluster_col])
                    .mean()
                    .sort_index()
                    .values
                )
        ...
        self.num_clusters_each_label = (
            df.groupby(self.label_col).nunique()[self.cluster_col].sort_index().values
        )
        self.dbg_df = df
        ...
        self._calc_high_dim_clusters_properties(
            df, self.label_col, self.cluster_col, intra_centroids_df
        )
    ...
    def _calc_high_dim_clusters_properties(self, df, label_col, cluster_col, intra_centroids_df):
        ...
        self.anchors_density = (
            df.groupby([label_col])[cluster_col].value_counts().sort_index().values
        )
        # Radius
        ...
        radius_df = radius_df.groupby(level=[0, 1]).quantile(
            q=self.radius_q if self.radius_q is not None else 1
        )
    ...
    def _anchor_to_label_cluster(self, anchor_index, visualization=False):
        ...
        if visualization and self.class_to_label is not None:
            return self.class_to_label[
                len(self.num_clusters_each_label) - 1
            ], anchor_index - anchors_count
        else:
    ...
    def _calc_inter_class_relations(self):
        ...
        :return: None
        """
        self.inter_class_relations = np.zeros(
            (len(self.intra_class_anchors_labels), len(self.intra_class_anchors_labels))
        )
        num_labels = len(np.unique(self.intra_class_anchors_labels))
        ...
        for x1, x2 in zip(edges_x1, edges_x2):
            ...
            self.inter_class_relations_label_level[self.y_with_centroids[x1]][
                self.y_with_centroids[x2]
            ] += 1
        ...
        self.inter_class_relations_label_level = (
            self.inter_class_relations_label_level / sum_row_label_level
        )
            ...
            for label_i in range(num_labels):
                ...
                for sample_index in np.argwhere(y_predicted != self.y_with_centroids):
                    ...
                    src_label, target_label = (
                        self.y_with_centroids[sample_index],
                        y_predicted[sample_index],
                    )
                    anchor = self._sample_index_to_anchor(
                        int(self.y_with_centroids[sample_index]), self.clusters[sample_index]
                    )
    ...
    def _calc_low_dim_inter_class_relations(self):
        ...
        for x1, x2 in zip(edges_x1, edges_x2):
            ...
            self.inter_class_relations_low_dim_label_level[self.y_with_centroids[x1]][
                self.y_with_centroids[x2]
            ] += 1
        if not self.self_relation:
        ...
        sum_row_label_level = self.inter_class_relations_low_dim_label_level.sum(
    ...
    def _dim_reduction(self):
        ...
        if self.dim_reduction_algo == "t-sne":
            dim_reduction_algo_inst = TSNE(
                random_state=self.random_state, perplexity=self.tsne_perplexity
            )
            ...
            dim_reduction_algo_inst = MDS(
                n_components=self.n_components, random_state=self.random_state
            )
        elif self.dim_reduction_algo == "pca":
            dim_reduction_algo_inst = PCA(
                n_components=self.n_components, random_state=self.random_state
            )
        elif self.dim_reduction_algo == "lda":
            dim_reduction_algo_inst = LatentDirichletAllocation(
                n_components=self.n_components, random_state=self.random_state
            )
        else:
            raise Exception(
                f"Dimension reduction algorithm {self.dim_reduction_algo} is not supported"
            )
        if self.supervised:
            self.logger.info("Supervised Dim Reduction")
            if self.reduce_all_points:
                self.logger.info("Dim Reduction all points")
                self.low_dim_points = dim_reduction_algo_inst.fit_transform(
                    self.X_with_centroids, self.y_with_centroids
                )
            else:
        ...
        if self.reduce_all_points:
            ...
            for label in sorted(contours_df[self.label_col].unique()):
                points = contours_df[contours_df[self.label_col] == label][
                    [self.x_col, self.y_col]
                ].values
                label_to_contour_df[label] = self._get_concave_hull(
                    points,
                ...
                for anchor, anchor_label in zip(
                    self.low_dim_anchors, self.intra_class_anchors_labels
                ):
                    anchor_index += 1
                ...
                for region in regions:
                    ...
                    shape[0] += 1
                    p = Polygon(
                        np.append(polygon_region, polygon_region[0]).reshape(*shape)
                    ).intersection(mask)
                    if p.is_empty or isinstance(p, Point):
                        self.logger.warning(
                            "Intersection of voronoy with polygon is empty or in single point"
                        )
    ...
    def _random_point_low_dim(self, i, low_dim_points, concave_hulls):
        """
    ...
    def _random_points_per_cluster(self, anchor_index, number_of_random_points=None):
        ...
        if self.random_points_method == "voronoi":
            ...
            for i, r in enumerate(self.anchor_voronoi_regions):
                if self.anchor_voronoi_regions_label[i] == self.intra_class_anchors_labels[
                    anchor_index
                ] and (
                    r.contains(low_dim_anchor) or r.intersects(low_dim_anchor)
                ):  # in case that the anchor intersects the exterior polygon
                    _poly = r
                    break
            if _poly is None:
    ...
    def _get_top_anchors_to_relax(self):
        ...
        src_anchor_indices, target_anchor_indices = np.unravel_index(
            np.argsort(inter_class_relation_loss, axis=None), inter_class_relation_loss.shape
        )
        # revers indices
        ...
        return (
            src_anchor_indices[: self.top_greedy],
            target_anchor_indices[: self.top_greedy],
            directions[: self.top_greedy],
            magnitudes[: self.top_greedy],
        )
    ...
    def _mask_inter_class_relations(self):
        ...
        if self.mask_sparse_subcluster is not None:
            mask = np.tile(
                self.anchors_density < self.mask_sparse_subcluster, (len(self.anchors_density), 1)
            ).T
    ...
    def _relax_anchor_cluster(self, src_anchor_index, target_anchor_index, direction, magnitude):
        ...
        """
        if (
            self.mask_sparse_subcluster is not None
            and self.anchors_density[src_anchor_index] < self.mask_sparse_subcluster
        ):
            self.logger.debug(
        ...
        src_anchor, target_anchor = (
            self.low_dim_anchors[src_anchor_index],
        ...
        self.low_dim_points[label_cluster_indices] = (
            self.low_dim_points[label_cluster_indices] + direction * magnitude * direction_vec
        )
        # update also the low dim anchors
        self.low_dim_anchors[src_anchor_index] = (
            self.low_dim_anchors[src_anchor_index] + direction * magnitude * direction_vec
        )
    ...
    def _relaxation(self):
        ...
        for i in range(self.n_iter):
            ...
            inter_class_relations, inter_class_relations_low_dim = (
                self._mask_inter_class_relations()
            )
            loss = self.loss_func(inter_class_relations, inter_class_relations_low_dim)
            if loss < self.stop_criteria:
                self.logger.info(
                    f"loss {loss} < stopping criteria {self.stop_criteria} nothing to do"
                )
                return
            ...
            for j in range(self.batch_size):
                src_anchor_indices, target_anchor_indices, directions, magnitudes = (
                    self._get_top_anchors_to_relax()
                )
                for ra_i in range(len(src_anchor_indices)):
                    self._relax_anchor_cluster(
                        src_anchor_indices[ra_i],
                        target_anchor_indices[ra_i],
                        directions[ra_i],
                        magnitudes[ra_i],
                    )
            ...
            with imageio.get_writer(gif_path, mode="I") as writer:
                for i in saved_iterations_for_gif:
                    for j in range(5):
                        writer.append_data(
                            imageio.imread(
                                f"{self.output_dir}/iter_{i}_points_anchors_patches_plot.png"
                            )
                        )

    def _cluster_plot_set_sns(
        self,
        i,
        figsize: tuple = (21, 15),
        show_blobs: bool = True,
        ...
        self._anchors_plot_sns_separate(
            i,
            figsize=figsize,
            show_blobs=show_blobs,
            show_points=show_points,
    ...
    def _anchors_plot_sns_separate(
        self,
        i,
        figsize: tuple = (21, 15),
        show_blobs: bool = True,
        ...
        if self.class_to_label:
            df[self.label_col] = df[self.label_col].transform(
                lambda x: f"{x}_{self.class_to_label[x]}"
            )
        else:
        ...
        self._points_anchors_patches_plot(
            df,
            i,
    ...
    def _matrices_plot(
        ...
        anchor_level_ticks = [
            str(self._anchor_to_label_cluster(i, visualization=True))
        ...
        if label_level:
            matrices = {
                "Overlap Diff": {
                    ...
                    "ticks": label_level_ticks,
                },
                "Overlap High-Dim": {
                    "mat": self.inter_class_relations_label_level,
                    "ticks": label_level_ticks,
                },
                "Overlap Low-Dim": {
                    "mat": self.inter_class_relations_low_dim_label_level,
                    "ticks": label_level_ticks,
                },
                "Proximity Diff": {
                    "mat": np.clip(
                ...
                "Proximity High-Dim": {
                    "mat": self.high_dim_proximity_matrix,
                    "ticks": label_level_ticks,
                },
                "Proximity Low-Dim": {
                    "mat": self.low_dim_proximity_matrix,
                    "ticks": label_level_ticks,
                },
            }
        else:
            matrices = {
                "Overlap Diff": {
                    ...
                    "ticks": anchor_level_ticks,
                },
                "Overlap High-Dim": {
                    "mat": self.inter_class_relations,
                    "ticks": anchor_level_ticks,
                },
                "Overlap Low-Dim": {
                    "mat": self.inter_class_relations_low_dim,
                    "ticks": anchor_level_ticks,
                },
                "Proximity Diff": {
                    "mat": np.clip(
        ...
        for i, row in enumerate(rows):
            for j, col in enumerate(cols):
                ...
                g = sns.heatmap(
                    ...
                    xticklabels=ticks,
                    yticklabels=ticks,
                    annot_kws={"size": mat_annot_kws_size},
                )
                ...
                if self.save_fig and self.every_matrix_in_single_plot:
                    ...
                    fig.savefig(
                        f"{self.output_dir}/{matrix_key}_iter_{iteration}.png",
    ...
    def _points_anchors_patches_plot(
        self,
        df,
        i,
        show_inner_blobs,
        ...
        if show_points:
            sns.scatterplot(
                data=df[df["anchor"] == False],
                x=self.x_col,
                y=self.y_col,
                hue=self.label_col,
                style=self.cluster_col if show_anchors else None,
                ax=ax,
                alpha=0.2 if show_blobs else 1,
                legend=False,
                hue_order=hue_order,
                s=240,
            )
        ...
        if show_anchors_annotation:
            for j, anchor_i in enumerate(self.anchors_indices):
                ax.text(
                    self.low_dim_points[anchor_i][0],
                    self.low_dim_points[anchor_i][1],
        ...
        if show_blobs:
            ...
            for label in sorted(contours_df[self.label_col].unique()):
                ...
                concave_hulls = self._get_concave_hull(
                    points,
                    alpha=self.alpha[label],
                    remove_outliers_k=self.remove_outliers_k,
                    spline=self.use_spline,
                ...
                for concave_hull in concave_hulls:
                    ...
                    if Polygon(concave_hull).area < self.skip_polygons_with_area:
                        self.logger.debug(
                            f"Skipping polygon of label {label} with area {Polygon(concave_hull).area}"
                        )
                        continue
                    coords = concave_hull
                    ...
                    patch = patches.PathPatch(
                        path, facecolor=c, alpha=0.2, linewidth=None, edgecolor=None
                    )
                    ax.add_patch(patch)
                    patch = patches.PathPatch(
                        path, facecolor=None, linewidth=10, edgecolor=c, fill=False
                    )
                    ax.add_patch(patch)
            ...
            if show_inner_blobs:
                ...
                for label in sorted(contours_df[self.label_col].unique()):
                    ...
                    for region in regions:
                        ...
                        shape[0] += 1
                        p = Polygon(np.append(polygon, polygon[0]).reshape(*shape)).intersection(
                            mask
                        )
                        ...
                        for p in list(polygons):
                            ...
                            if p.exterior is None:
                                self.logger.debug(
                                    f"Skipping voronoi of anchor of label {label} since it is outside "
                            ...
                            line_cmde = (
                                [Path.MOVETO]
                                + [Path.LINETO] * (len(inner_blobs) - 2)
                                + [Path.CLOSEPOLY]
                            )
                            path = Path(inner_blobs, line_cmde)
                            patch = patches.PathPatch(
                                path, facecolor=None, linewidth=5, edgecolor=c, fill=False
                            )
                            ax.add_patch(patch)
            ...
            if annotate_images:
                ...
                for outlier_label, anchor in enumerate(self.overlapped_anchor_per_label):
                    ...
                    target_label = self.class_to_label[
                        self.cand_samples_to_plot[anchor]["target_label"]
                    ]
                    if outlier_is_valid:
                        self.logger.info(
                        ...
                        sns.scatterplot(
                            x=[self.low_dim_anchors[anchor][0]],
                            y=[self.low_dim_anchors[anchor][1]],
                            c=[c],
                            s=480,
                        )
        ...
        if show_blobs:
            if not annotate_images:
                lgd = plt.legend(
                    current_handles[: num_labels + 1],
                    current_labels[: num_labels + 1],
                ...
                if self.orig_images:
                    lgd = plt.legend(
                        current_handles[1 : num_labels + 1],
                        current_labels[1 : num_labels + 1],
    ...
    def _get_contour_df(self):
        ...
        x_plus_arr[:, 0] = low_dim_arr[:, 0] + (
            self.anchors_radius if self.radius_q is not None else 0.01
        )
        x_minus_arr = low_dim_arr.copy()
        x_minus_arr[:, 0] = low_dim_arr[:, 0] - (
            self.anchors_radius if self.radius_q is not None else 0.01
        )
        y_plus_arr = low_dim_arr.copy()
        y_plus_arr[:, 1] = low_dim_arr[:, 1] + (
            self.anchors_radius if self.radius_q is not None else 0.01
        )
        y_minus_arr = low_dim_arr.copy()
        y_minus_arr[:, 1] = low_dim_arr[:, 1] - (
            self.anchors_radius if self.radius_q is not None else 0.01
        )
        if self.n_components > 2:
            z_plus_arr = low_dim_arr.copy()
            z_plus_arr[:, 2] = low_dim_arr[:, 2] + (
                self.anchors_radius if self.radius_q is not None else 0.01
            )
            z_minus_arr = low_dim_arr.copy()
            z_minus_arr[:, 2] = low_dim_arr[:, 2] - (
                self.anchors_radius if self.radius_q is not None else 0.01
            )
        anchors_radius = np.concatenate(
            [low_dim_arr, x_plus_arr, x_minus_arr, y_plus_arr, y_minus_arr]
        )
        if self.n_components > 2:
        ...
        anchors_df = pd.DataFrame(
            anchors_radius,
            columns=[self.x_col, self.y_col, "z"]
            if self.n_components > 2
            else [self.x_col, self.y_col],
        )
    ...
    def _smooth_poly_Chaikins_corner_cutting(poly, append_first_point):
        ...
        for i in range(len(poly) - 1):
            q_i = [
                0.75 * poly[i][0] + 0.25 * poly[i + 1][0],
                0.75 * poly[i][1] + 0.25 * poly[i + 1][1],
            ]
            r_i = [
                0.25 * poly[i][0] + 0.75 * poly[i + 1][0],
                0.25 * poly[i][1] + 0.75 * poly[i + 1][1],
            ]
    ...
    def _get_concave_hull(
        ...
        for shape in list(alpha_shape):
            ...
            if not spline:
                if vis:
                    smooth_shape = np.array(
                        ClusterPlot._smooth_poly_Chaikins_corner_cutting_iter(
                            ClusterPlot._smooth_poly_Douglas_Peucker(
                                list(zip(x, y)), douglas_peucker_tolerance
                            ),
