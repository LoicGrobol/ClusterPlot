...
class DataSet:
    ...
    def __init__(
        self, df, feature_cols, label_col, class_to_label: dict = None, orig_images: list = None
    ):
        self.df = df
...
class DataSetFactory:
    ...
    def get_dataset(dataset_name, random_state=None, sample=None, is_subset=False):
        ...
        if dataset_name == "MNIST_AE_ORIG":
            return DataSetFactory.mnist_ae(
                dim=None, sample=sample, is_subset=True, random_state=random_state
            )
        if dataset_name == "MNIST_AE32":
            return DataSetFactory.mnist_ae(
                dim=32, sample=sample, is_subset=True, random_state=random_state
            )
        if dataset_name == "MNIST_AE4":
            return DataSetFactory.mnist_ae(
                dim=4, sample=sample, is_subset=True, random_state=random_state
            )
        ...
        if dataset_name in [
            ...
            return DataSet(
                ...
                feature_cols,
                label_col,
                class_to_label={label: f"Poly{label}" for label in df[label_col].unique()},
            )
        else:
